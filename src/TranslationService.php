...
final class TranslationService
{
	 ...
	 * Uses a single upsert (`ON DUPLICATE KEY UPDATE` on MySQL, `ON CONFLICT` on PostgreSQL).
	...
	private function choosePluralForm(string $value, int $count): string
	{
		...
		return $count === 1 ? Str::before($value, '|') : Str::after($value, '|');
	}
}
